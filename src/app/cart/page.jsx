        ...
        {step==="cart"&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 340px",gap:"28px"}}>
            ...
            <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:"16px",padding:"24px",height:"fit-content",position:"sticky",top:"80px"}}>
              ...
              <button onClick={()=>cart.length>0&&setStep("address")}
          ...
          <div style={{maxWidth:"520px",margin:"0 auto"}}>
            ...
            <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:"14px",padding:"20px",marginBottom:"24px"}}>
              {[["Items",`₹${sub.toLocaleString()}`],["Gift Wrap","FREE"],["Shipping",ship===0?"FREE":`₹${ship}`]].map(([l,v])=>(
                <div key={l} style={{display:"flex",justifyContent:"space-between",marginBottom:"8px",color:MUTED,fontSize:"14px"}}><span>{l}</span><span style={{color:v==="FREE"?"#52b788":"inherit"}}>{v}</span></div>
              ))}
            ...
            <div style={{display:"flex",gap:"12px"}}>
              <button onClick={()=>setStep("address")} style={{flex:1,background:"transparent",border:`1px solid ${BORDER}`,borderRadius:"8px",padding:"13px",color:MUTED,fontFamily:"'Nunito',sans-serif",fontWeight:600,cursor:"pointer"}}>← Back</button>
              <button onClick={()=>setStep("success")} style={{flex:2,background:GOLD,border:"none",borderRadius:"8px",padding:"13px",fontFamily:"'Nunito',sans-serif",fontWeight:800,fontSize:"15px",cursor:"pointer",color:DARK,letterSpacing:"0.5px"}}>Pay ₹{total.toLocaleString()} →</button>
            </div>
          </div>
        )}
      </div>
