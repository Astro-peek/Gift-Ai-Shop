  ...
  return(
    <div style={{fontFamily:"'Nunito',sans-serif",minHeight:"100vh",background:DARK,color:"#F0EAD6"}}>
      ...
      <div style={{maxWidth:"1100px",margin:"0 auto",padding:"48px 2rem"}}>
        ...
        {orders.length === 0 ? (
          <div style={{textAlign:"center",padding:"100px 0",background:CARD,borderRadius:"16px",border:`1px solid ${BORDER}`}}>
            <div style={{fontSize:"48px",color:GOLD,marginBottom:"20px",fontFamily:"'Cormorant Garamond',serif"}}>◇</div>
            <div style={{fontSize:"22px",fontFamily:"'Cormorant Garamond',serif",marginBottom:"12px"}}>No orders yet</div>
            <a href="/" style={{color:GOLD,fontWeight:700,textDecoration:"none",fontSize:"14px"}}>Start Gifting →</a>
          </div>
        ) : (
          <div style={{display:"grid",gridTemplateColumns:"340px 1fr",gap:"24px"}}>
            <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
              {orders.map(o=>(
                <div key={o.id} onClick={()=>setSel(o)} style={{background:sel?.id===o.id?"#1e1a0e":CARD,border:`1px solid ${sel?.id===o.id?GOLD+"55":BORDER}`,borderRadius:"14px",padding:"18px",cursor:"pointer",transition:"all 0.15s",display:"flex",gap:"12px"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:"12px",color:MUTED,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{o.product}</div>
                  </div>
                </div>
              ))}
            </div>
                  ...
                  <div style={{display:"flex",alignItems:"center"}}>
                    {sel.steps.map((s,i)=>(
                      <div key={s} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",position:"relative"}}>
                        ...
                        <div style={{fontSize:"11px",marginTop:"8px",fontWeight:700,color:i<=sel.cur?GOLD:MUTED,textTransform:"capitalize"}}>{s}</div>
                      </div>
                    ))}
                  </div>
                </div>
